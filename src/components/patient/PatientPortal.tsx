      ...
      <main className="max-w-2xl mx-auto w-full p-4 md:p-6 space-y-6 flex-1">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-md p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Now Serving */}
            <div className="bg-emerald-800 text-white rounded-xl p-4 border border-emerald-700 shadow-xs">
              <span className="text-xs font-bold text-white uppercase tracking-widest block mb-2">
                NOW SERVING
              </span>
              {publicQueue.nowServing.length === 0 ? (
                <p className="text-base text-emerald-50 italic font-medium py-2">No patient currently being served.</p>
              ) : (
                <div className="space-y-2">
                  {publicQueue.nowServing.map(t => (
                    <div key={t.id} className="p-3 bg-white rounded-xl border border-emerald-200 flex justify-between items-center shadow-xs">
                      <div>
                        <span className="font-mono font-black text-2xl text-emerald-800 block tracking-tight">{t.tokenNumber}</span>
                        {t.doctorName && <span className="text-xs font-semibold text-slate-700 block">{t.doctorName}</span>}
                      </div>
                      <span className="text-xs font-bold text-slate-900 bg-slate-100 px-2.5 py-1 rounded-lg border border-slate-200">{t.roomNumber}</span>
                    </div>
                  ))}
                </div>
              )}
